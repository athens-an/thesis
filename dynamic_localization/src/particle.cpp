...
void Particle::setParticleWeight(unsigned int width, unsigned int height,
  int** data, float resolution, const std::vector<float> &ranges, const std::vector<float> &ranges_id, 
  ...
  std::vector<std::vector<float> > rfid_pose, int subs_step, float strict)
{
  ...
  float sum = 0;
	...
	//~ ROS_INFO_STREAM("EGINEEEEEE " << ranges.size() << " " << ranges[100]);
  for (unsigned int i = 0 ; i < ranges.size(); i+= subs_step)
  {
    getRanges(
      ranges_id[i] * increment + angle_min + _theta, 
      width, 
      height, 
      data, 
      resolution, 
